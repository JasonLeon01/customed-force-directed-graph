...
with open('result_0830_S7.json') as file:
    data = json.load(file)
    colNum = data['table']['colNum']
    rowNum = data['table']['rowNum']
    rowDict = listToDict(data['table']['rows'])
    colDict = listToDict(data['table']['cols'])
    # 只对当前focus 的点做计算
    nodeData = list(
        filter(lambda x: x['state'] == 'S7', data['graph']['nodes'])
    )
